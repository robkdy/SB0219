...
ON 
	b.facid = f.facid
WHERE
	LEFT(b.starttime, 10) = '2012-09-14'
HAVING Cost > 30
ORDER BY
	Cost DESC
...
SELECT sub.name as Facility,
  CONCAT(sub.firstname, ' ',sub.surname) as Member,
	CASE WHEN sub.memid = 0 THEN sub.guestcost * sub.slots ELSE sub.membercost * sub.slots END as Cost
FROM 
  (SELECT f.name, m.firstname, m.surname, b.memid, f.guestcost, b.slots, f.membercost
  FROM
	country_club.Members m
  JOIN
	country_club.Bookings b
